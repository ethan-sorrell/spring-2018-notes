...
start :- ask_level(L), L>=1, L=<3, retractall(level(_)), asserta(level(L)), show_rules, init(9, B), x_turn(B).

ask_level(L) :- write('Enter level 1 (beginner), 2 (intermediate), or 3 (advanced) followed by dot.'), nl, read(L).

show_rules :- write('You are X. Enter integer 1 to 9 followed by dot.'), nl, draw_board([1,2,3,4,5,6,7,8,9]).

draw_board([A,B,C,D,E,F,G,H,I]) :- write([A,B,C]), nl, write([D,E,F]), nl, write([G,H,I]), nl, nl.

init(0, [ ]).
init(N, ['-'|X]) :- N>0, M is N-1, init(M, X).

x_turn(B) :- won(o, B), write('Computer wins.'), nl, !.
x_turn(B) :- read(K), move(x, B, K, C), draw_board(C), !, o_turn(C).
...
advanced_move(B) :- will_win(o, B, K), move(o, B, K, C), draw_board(C), !, x_turn(C).
advanced_move(B) :- will_win_or_tie(o, B, K), move(o, B, K, C), draw_board(C), !, x_turn(C).
advanced_move(B) :- move(o, B, _, C), draw_board(C), !, x_turn(C).

intermediate_move(B) :- one_move_win(o, B, C), draw_board(C), !, x_turn(C).
intermediate_move(B) :- move(o, B, _, C), not(one_move_win(x, C, _)), draw_board(C), !, x_turn(C).
intermediate_move(B) :- move(o, B, _, C), draw_board(C), !, x_turn(C).

beginner_move(B) :- move(o, B, _, C), draw_board(C), !, x_turn(C).

not(X) :- \+ X.         % GNU Prolog does not provide the built-in not predicate
